...
void
SiStripGainsValidator::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  using namespace edm;
...
void 
SiStripGainsValidator::beginJob()
{
  ...
  for ( int fooInt = SiStripHelper::TIBL1; fooInt != SiStripHelper::NUM_OF_TYPES; fooInt++ ){
    ...
    h_StoN_layer[layer]                  = StoN.make<TH1F>(Form("clusterStoN_%s",append.c_str()),Form("cluster Raw S/N (%s); %s cluster raw S/N;# clusters",s_layer.c_str(),s_layer.c_str()),100,0.,100.);
    h_StoNCorr_layer[layer]              = StoN.make<TH1F>(Form("clusterStoNCorr_%s",append.c_str()),Form("cluster S/N (%s) corrected for path lenght;%s cluster S/N;# clusters",s_layer.c_str(),s_layer.c_str()),100,0.,100.);
    h_CChargeOverPath_layer[layer]       = ClusterCharge.make<TH1F>(Form("clusterChargeOverPath_%s",append.c_str()),Form("cluster charge over path (%s); %s cluster charge / path;# clusters",s_layer.c_str(),s_layer.c_str()),100,0.,1000.);
    h_CChargeOverPathNewG2_layer[layer]  = ClusterCharge.make<TH1F>(Form("clusterChargeOverPathNewG2_%s",append.c_str()),Form("cluster charge over path (new G2) (%s); %s cluster charge / path;# clusters",s_layer.c_str(),s_layer.c_str()),100,0.,1000.);
    h_CChargeOverPathNoG2_layer[layer]   = ClusterCharge.make<TH1F>(Form("clusterChargeOverPathNoG2_%s",append.c_str()),Form("cluster charge over path (G2 removed)(%s); %s cluster charge / path;# clusters",s_layer.c_str(),s_layer.c_str()),100,0.,1000.);
    h_CChargeOverPathNoG1_layer[layer]   = ClusterCharge.make<TH1F>(Form("clusterChargeOverPathNoG1_%s",append.c_str()),Form("cluster charge over path (G1 removed)(%s); %s cluster charge / path;# clusters",s_layer.c_str(),s_layer.c_str()),100,0.,1000.);
    h_CChargeOverPathNoG1G2_layer[layer] = ClusterCharge.make<TH1F>(Form("clusterChargeOverPathNoG1G2_%s",append.c_str()),Form("cluster charge over path (all gains removed)(%s); %s cluster charge / path;# clusters",s_layer.c_str(),s_layer.c_str()),100,0.,1000.);
...
//****************************************************************/
bool SiStripGainsValidator::IsFarFromBorder(TrajectoryStateOnSurface* trajState, const uint32_t detid, const edm::EventSetup* iSetup)
//****************************************************************/
{ 
...
//****************************************************************/
std::string SiStripGainsValidator::myreplace(const std::string &s,const std::string &toReplace,const std::string &replaceWith)
//****************************************************************/
{
...
{ 
  ...
  if( InputHisto->GetEntries() < 10.)return;
...
void
SiStripGainsValidator::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
