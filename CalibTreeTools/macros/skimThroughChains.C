void skimThroughChains(UInt_t run_number = 283946) {
  ...
  TSystemDirectory dir(
      "root://eoscms.cern.ch//store/group/dpg_tracker_strip/comm_tracker/Strip/Calibration/calibrationtree/"
      "GR16_ReReco_old/",
      "root://eoscms.cern.ch//store/group/dpg_tracker_strip/comm_tracker/Strip/Calibration/calibrationtree/"
      "GR16_ReReco_old/");
  ...
  for (Int_t i = 0; i < nentries; i++) {
    ...
    for (Int_t j = 0; j < 100; j++) {
      if (i % nentries == j * (nentries / 100))
        std::cout << i << " |fraction of hit eff tree: " << j << " % completed" << std::endl;
    }
    if (run == run_number)
      newtree->Fill();
  }
  ...
  for (Int_t k = 0; k < nentries2; k++) {
    ...
    for (Int_t l = 0; l < 100; l++) {
      if (k % nentries == l * (nentries2 / 100))
        std::cout << k << " |fraction of lumi tree : " << l << " % completed" << std::endl;
    }
    if (run == run_number)
      newtree2->Fill();
  }
