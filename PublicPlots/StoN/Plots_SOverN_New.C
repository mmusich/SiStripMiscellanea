...
TH1* GetSOverNHisto(TDirectory *current_sourcedir)
{
  ...
  while ( (key = (TKey*)nextkey())) {
    ...
    if ( obj->IsA()->InheritsFrom( TH1::Class() ) ) {
	 ...
	 !obj_name.Contains("Summary_ClusterStoNCorr__OnTrack__")) continue;
...
void DrawSOverNHisto(TCanvas *& c, TH1* h, int run, string subdet, bool thin=true)
{
  ...
  if(subdet=="TOB") fit->SetRange(17.5,40);
  if(subdet=="TEC") fit->SetRange(12.5,40);
  if(subdet=="TIB") fit->SetRange(10.5,30);
  if(subdet=="TID") fit->SetRange(10.5,30);
  ...
  if(run==262235) text->AddText("2015 Data, sqrt(s)=5TeV");
  ...
  if(subdet=="TEC")
    {
      if(thin) text->AddText(Form("%s thin MPV: %.1f", subdet.c_str(), mpv));
...
void Plot(TCanvas *c, TFile* f, int run, string subdet="TIB", bool thin=true)
{
  ...
  if(subdet=="TEC"){
	...
	f->cd(Form("DQMData/Run %i/SiStrip/Run summary/MechanicalView/TEC/%s/ring_%i",
		   run, sides[iside].c_str(), iring));
	...
	if(iring<5) {
	  if(!hTECthin) hTECthin = h;
	  else hTECthin->Add(h);
	}
	else {
	  if(!hTECthick) hTECthick = h;
	  else hTECthick->Add(h);
	}
      } 
    }
  }
  // For the other subdets (TIB, TOB, TID)
    ...
    if(subdet=="TID") gDirectory->cd("MINUS");
  ...
  if(!h && !hTECthin && !hTECthick) return;
  if(thin && !h && hTECthin) h=hTECthin;
  if(!thin && !h && hTECthick) h=hTECthick;
 
  // Draw histos
  if(h) DrawSOverNHisto(c, h, run, subdet, thin);
  c->Modified();
  c->Update();
...
{
  int nlayer=4;
  if(subdet=="TIB") nlayer=4;
  if(subdet=="TOB") nlayer=6;
  ...
  for(int ilayer=1; ilayer<nlayer+1; ilayer++){
    ...
    if(subdet=="TOB") fit->SetRange(15.5,100);
    else fit->SetRange(15.5,100.);
    h->Fit("mylandau", "R");
    double mpv = fit->GetParameter(1);
    double mpv_err = fit->GetParError(1);
    fit->Draw("same");
...
void Plots_SOverN_New(string file="DQM_V0001_R000278803__ZeroBias__Run2016F-PromptReco-v1__DQMIO.root")
{
  ...
  for(int ilayer=0; ilayer<4; ilayer++) glayer->SetPoint(ilayer, ilayer+1, MPV_TIB[ilayer]);
  for(int ilayer=0; ilayer<6; ilayer++) glayer->SetPoint(ilayer+4, ilayer+5, MPV_TOB[ilayer]);
