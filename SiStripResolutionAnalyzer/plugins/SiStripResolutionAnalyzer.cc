...
public:
  ...
  ~SiStripResolutionAnalyzer() override;

  static void fillDescriptions(edm::ConfigurationDescriptions& descriptions);
  void fillHitQuantities(const Trajectory* trajectory,
                         std::vector<TrackerValidationVariables::AVHitStruct>& v_avhitout,
                         bool isBiased);
...
SiStripResolutionAnalyzer::SiStripResolutionAnalyzer(const edm::ParameterSet& iConfig)
    : trajCollectionToken_(
          consumes<std::vector<Trajectory>>(edm::InputTag(iConfig.getParameter<std::string>("trajectoryInput")))),
...
void SiStripResolutionAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  using namespace edm;
...
void SiStripResolutionAnalyzer::beginJob() {}

// ------------ method called once each job just after ending the event loop  ------------
void SiStripResolutionAnalyzer::endJob() {}

// ------------ method called once each job just after ending the event loop  ------------
void SiStripResolutionAnalyzer::fillHitQuantities(const Trajectory* trajectory,
                                                  std::vector<TrackerValidationVariables::AVHitStruct>& v_avhitout,
                                                  bool isBiased) {
  ...
  for (std::vector<TrajectoryMeasurement>::const_iterator itTraj = tmColl.begin(); itTraj != tmColl.end(); ++itTraj) {
    ...
    if (errHit.xx() < 0. || errHit.yy() < 0. || errTrk.xx() < 0. || errTrk.yy() < 0.) {
      edm::LogError("SiStripResolutionAnalyzer")
          << "@SUB=SiStripResolutionAnalyzer::fillHitQuantities"
    ...
    if (hit->detUnit()) {  // is it a single physical module?
      ...
      if (IntSubDetID == PixelSubdetector::PixelBarrel || IntSubDetID == StripSubdetector::TIB ||
        ...
        if (rectangularBound != nullptr) {
          ...
          throw cms::Exception("Geometry Error") << "[SiStripResolutionAnalyzer] Cannot cast bounds to "
...
void SiStripResolutionAnalyzer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
